            ...
            <tbody>
              {[
                ...
                <tr key={row[0]} style={{ borderBottom: '1px solid var(--color-neutral-stroke-divider)' }}>
                  {row.map((cell, i) => (
                    <td key={i} style={{ padding: '8px 16px', textAlign: 'center', color: i === 0 ? 'var(--color-neutral-foreground-subtle-1)' : 'var(--color-neutral-foreground-default)', fontWeight: i === 0 ? 600 : 400 }}>
                      {cell}
                    </td>
                  ))}
                </tr>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {(
            ...
            <div key={size} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <span style={{ fontFamily: 'Pretendard, sans-serif', fontSize: 12, color: 'var(--color-neutral-foreground-subtle-1)' }}>{desc}</span>
            </div>
          ))}
        </div>
      </Block>
        ...
        <table style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              {sizes.map(s => (
                <th key={s} style={{ padding: '8px 16px', fontSize: 11, color: 'var(--color-neutral-foreground-subtle-3)', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase' }}>
                  {s}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {variants.map(v => (
              <tr key={v} style={{ borderTop: '1px solid var(--color-neutral-stroke-divider)' }}>
